...
public:
    ...
    // the entire range is copied to a new, bigger memory MemoryChunk, and the
    // value of 'range_start' is updated.
    ...
    char* allocContinue(size_t additional_bytes, char const*& range_start,
        ...
        // Allocating zero bytes doesn't make much sense. Also, a zero-sized range
        // might break the invariant that the range begins at least before the
        // current MemoryChunk end.
        ...
        // Extend an exist memory range with 'additional_bytes'.
        ...
        //
        // Note: among other things, this method is used to provide a hick-ish
        // implementation of realloc over ArenaAllocators. It wastes a lot of
        // memory -- quadratically so when we reach the linear allocation threshold.
        // This deficiency is intentionally left as is, and should be solved not
        // by complicating this method, but by rethinking the approach to memory
        // management for aggregate function states, so that we can provide a
        // proper realloc().
