...
public:
    ...
    void free(void* buf, size_t size) {
        try {
            ...
            std::cout << "Allocator free failed" << std::endl;
            throw;
        }
    }
    ...
    void* realloc(void* buf, size_t old_size, size_t new_size, size_t alignment = 0) {
        ...
        if (old_size == new_size) {
        ...
        } else if (new_size < MMAP_THRESHOLD) { // and old_size > MMAP_THRESHOLD
            ...
            buf = new_buf;
        } else { // new_size > MMAP_THRESHOLD and old < MMAP_THRESHOLD
...
protected:
    ...
    // 的时候，如果需要的页面不在内存中，就会发生缺页中断，内核分配物理内存，如果是文件映射的
    // 话，顺手把文件读进来。
