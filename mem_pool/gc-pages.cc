...
void gc_init_page(void)
{
    ...
    if (GC_PAGE_SZ * block_pg_cnt < page_size)
        block_pg_cnt = page_size / GC_PAGE_SZ; // exact division
}
...
// exception is thrown.
static jl_gc_pagemeta_t *gc_alloc_new_page(void)
{
    // try to allocate a large block of memory (or a small one)
    unsigned pg, pg_cnt = block_pg_cnt;
    char *mem = NULL;
    while (1) {
        if (__likely((mem = gc_try_alloc_pages(pg_cnt))))
            break;
        size_t min_block_pg_alloc = MIN_BLOCK_PG_ALLOC;
        if (GC_PAGE_SZ * min_block_pg_alloc < page_size)
            min_block_pg_alloc = page_size / GC_PAGE_SZ; // exact division
    ...
    jl_gc_pagemeta_t *page_meta =
        (jl_gc_pagemeta_t*)gc_perm_alloc_nolock(pg_cnt * sizeof(jl_gc_pagemeta_t), 1,
                                                   sizeof(void*), 0);
    pg = 0;
    if (page_meta) {
        for (; pg < pg_cnt; pg++) {
            ...
            if (!info.pagetable1) {
                info.pagetable1 = (pagetable1_t*)gc_perm_alloc_nolock(sizeof(pagetable1_t), 1,
                                                                         sizeof(void*), 0);
            ...
            if (!info.pagetable0) {
                info.pagetable0 = (pagetable0_t*)gc_perm_alloc_nolock(sizeof(pagetable0_t), 1,
                                                                         sizeof(void*), 0);
    ...
    if (pg < pg_cnt) {
        ...
        munmap(mem + LLT_ALIGN(GC_PAGE_SZ * pg, page_size),
               GC_PAGE_SZ * pg_cnt - LLT_ALIGN(GC_PAGE_SZ * pg, page_size));
#endif
        if (pg == 0) {
            // uv_mutex_unlock(&gc_perm_lock);
            gc_perm_lock.unlock();
...
jl_gc_pagemeta_t *gc_alloc_page(void)
{
    ...
    {
        jl_gc_pagemeta_t *meta = gc_alloc_new_page();
...
void gc_free_page(void *p)
{
    ...
    if (GC_PAGE_SZ < page_size) {
        ...
        size_t n_pages = page_size / GC_PAGE_SZ; // exact division
        decommit_size = page_size;
        void *otherp = (void*)((uintptr_t)p & ~(page_size - 1)); // round down to the nearest physical page
        p = otherp;
        while (n_pages--) {
