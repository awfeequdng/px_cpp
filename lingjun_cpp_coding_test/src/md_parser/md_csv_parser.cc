...
uint64_t time1w(double val) {
    uint64_t t10w = val * 100000;
    if (t10w % 10 == 9) {
        // 类似于1.1 这个浮点数为1.099999, 那么t10w = 109999, t10w++后就位110000
        t10w++;
    }
    return t10w / 10;
}
...
void MarketDataCsvParser::parse_md(const std::string &fname) {
    ...
    if (!f.good()) {
        throw std::logic_error("file not exist: " + fname);
    }
    ...
    for (int i = 0; i < row_cnt; i++) {
        ...
        // todo: 这个问题后面处理
        tick->bid_px[0] =  time1w(doc.GetCell<double>("BPrice1", i));
        tick->ask_px[0] = time1w(doc.GetCell<double>("SPrice1", i));
        tick->bid_sz[0] = doc.GetCell<uint32_t>("BVol1", i);
        tick->ask_sz[0] = doc.GetCell<uint32_t>("SVol1", i);
        // 1
        tick->bid_px[1] = time1w(doc.GetCell<double>("BPrice2", i));
        tick->ask_px[1] = time1w(doc.GetCell<double>("SPrice2", i));
        tick->bid_sz[1] = doc.GetCell<uint32_t>("BVol2", i);
        tick->ask_sz[1] = doc.GetCell<uint32_t>("SVol2", i);
        // 2
        tick->bid_px[2] = time1w(doc.GetCell<double>("BPrice3", i));
        tick->ask_px[2] = time1w(doc.GetCell<double>("SPrice3", i));
        tick->bid_sz[2] = doc.GetCell<uint32_t>("BVol3", i);
        tick->ask_sz[2] = doc.GetCell<uint32_t>("SVol3", i);
        // 3
        tick->bid_px[3] = time1w(doc.GetCell<double>("BPrice4", i));
        tick->ask_px[3] = time1w(doc.GetCell<double>("SPrice4", i));
        tick->bid_sz[3] = doc.GetCell<uint32_t>("BVol4", i);
        tick->ask_sz[3] = doc.GetCell<uint32_t>("SVol4", i);
        // 4
        tick->bid_px[4] = time1w(doc.GetCell<double>("BPrice5", i));
        tick->ask_px[4] = time1w(doc.GetCell<double>("SPrice5", i));
