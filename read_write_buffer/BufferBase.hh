...
/**
 ...
 so that after reading, the position reamains immediately after the read value.
 ...
 - in the case of ReadBuffer - fill in the buffer whit new data from the source;
 ...
 In the second case, you can effectively read from an already existing piece of memory / std::string without copyint it.
*/
class BufferBase {
public:
    ...
    struct Buffer {
        ...
        inline std::size_t size() const { return std::size_t(end_pos - begin_pos); }
        inline void resize(std::size_t size) { end_pos = begin_pos + size; }
    ...
    BufferBase(Position ptr, std::size_t size, std::size_t offset)
        : pos(ptr + offset), working_buffer(ptr, ptr + size), internal_buffer(ptr, ptr + size) {}

    void set(Position ptr, std::size_t size, std::size_t offset) {
    ...
    // get (for reading and modifying) the position
    inline Position& position() { return pos; }

    // offset in bytes of the cursor from the beginning of the buffer
    inline std::size_t offset() const { return std::size_t(pos - working_buffer.begin()); }

    // How many bytes are available for read/write
    inline std::size_t available() const { return std::size_t(working_buffer.end() - pos); }
    ...
    std::size_t count() const { return bytes + offset(); }

    // Check that there is more bytes in buffer after cursor.
    bool inline hasPendingData() const { return available() > 0; }
